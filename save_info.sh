...
/bin/tar zcf /flash/`/bin/hostname`.tgz \
	etc/isakmpd etc/iked etc/ssh \
	etc/rc.local etc/rc.conf.local \
	usr/local

skipList="boot bsd bsd.mp etc new old openbsd.vnd var.tar `/bin/hostname`.tgz"
FLASH_SAVE_DIRS=" "
...
/bin/tar zcf /flash/`/bin/hostname`_flash.tgz ${FLASH_SAVE_DIRS}
